...
> [!IMPORTANT]
> **Open question:** each of the 5 boards per strip has its own NE555 pair, but that does **not** necessarily mean every board-to-board signal is independent — the documented DATA daisy-chain (QS1 of one board feeding DATA of the next, forming one continuous 200-bit shift register per bank) can only work if the **shift CLOCK (pin 8) and STROBE (pin 7)** are a single signal truly shared across all 5 boards. The local 555s may instead be driving something board-local (e.g. row address advance / output-enable timing) that doesn't need to be synchronized between boards.
>
> **TODO:** scope/continuity-check **pin 8 (CLOCK) and pin 7 (STROBE) specifically** on two adjacent boards. If those two signals are identical/phase-locked across boards, one shared CLOCK+STROBE per strip is enough regardless of what the local 555s are doing elsewhere. If they're genuinely independent per board, the MCU needs to track a separate clock domain per board (5 per strip, 20 total) — a much bigger design difference, worth ruling out before picking an MCU/architecture.
...
```
>  [!IMPORTANT]
> The two banks have **separate DATA lines**. The top and bottom module rows are driven independently, allowing them to display different content. CLOCK and STROBE are shared across both banks, but each bank requires its own serial data feed.
...
Connects the controller board to the LED board. Pin assignments documented here are as measured on the **LED board** connector.
...
| Left | Signal | Description | Right | Signal | Description |
|---------|--------|-------------|----------|--------|-------------|
| 1 | A1 | Row address bit 1 (HC238) | 2 | A0 | Row address bit 0 (HC238) |
| 3 | E3 | Enable Input (HC238) | 4 | A2 | Row address bit 2 (HC238) |
...
| 15 | VBB | 5V | 16 | VBB | 5V |
| 17 | VCC | 12V | 18 | VCC | 12V |
| 19 | GND | Ground | 20 | GND | Ground |

**Candidates for unknown pins:** OE (output enable -> HC4094/HC244Q), RESET.
